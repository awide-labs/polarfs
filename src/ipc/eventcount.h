...
/*
 * This file is slightly modified version of folly::EventCount with only difference
 * that it is using different futexWait/futexWake implementation. Folly's implementation
 * is using FUTEX_PRIVATE_FLAG which does not allow futex to be used for IPC.
 */
...
inline int futexWake(const void *addr, int count, uint32_t wakeMask) {
  const auto rv = syscall(__NR_futex, addr,  /* addr1 */
...
inline FutexResult futexWait(const void *addr, uint32_t expected,
                             struct timeval *abstime, uint32_t waitMask) {
...
inline void EventCount::notify() noexcept { doNotify(1); }

inline void EventCount::notifyAll() noexcept { doNotify(INT_MAX); }
...
inline void EventCount::wait(Key key) noexcept {
  while ((val_.load(std::memory_order_acquire) >> kEpochShift) == key.epoch_) {
    futexWait(reinterpret_cast<uint32_t *>(&val_) + kEpochOffset, key.epoch_,
              nullptr, -1);
  }
  // memory_order_relaxed would suffice for correctness, but the faster
  // #waiters gets to 0, the less likely it is that we'll do spurious wakeups
...
template <class Condition> void EventCount::await(Condition condition) {
  if (condition()) {
    return; // fast path
  }
