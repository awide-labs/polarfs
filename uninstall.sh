...
#uninstall check
exist_command="ps -ef | grep -E 'pfsdaemon|pfs-fuse' |  grep -v grep  | wc -l"
exist=$(eval $exist_command)
if [ $exist -ge 1 ]; then
	echo "pfsd or fuse is running, before uninstall pfsd, please stop pfsd and fuse"
	exit 1
fi
...
rm ${INSTALL_BASE_DIR}/pfsd/bin/pfs-fuse
